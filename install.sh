...
izin_line=$(curl -fsSL "$IZIN_URL" | awk -v ip="$MYIP" '$1 == ip {print; exit}')
if [[ -z "$izin_line" ]]; then
  echo -e "${RED}IP $MYIP belum terdaftar di izin: $IZIN_URL${NC}"
  exit 1
fi
username=$(awk '{print $2}' <<< "$izin_line")
expx=$(awk '{print $3}' <<< "$izin_line")
# Format file izin: IP USERNAME EXPIRED, contoh: 1.2.3.4 user 2026-12-31
if ! date -d "$expx" >/dev/null 2>&1; then
  echo -e "${RED}Format expired tidak valid. Pakai YYYY-MM-DD di kolom ke-3.${NC}"
  exit 1
fi
...
if [[ "$today" < "$expx" || "$today" == "$expx" ]]; then
  sts="${Info}"
else
  echo -e "${RED}Izin IP $MYIP sudah expired pada $expx${NC}"
  exit 1
fi
NET=$(ip route get 8.8.8.8 2>/dev/null | awk '{for(i=1;i<=NF;i++) if ($i=="dev") {print $(i+1); exit}}')
NET=${NET:-eth0}

clear
REPO="https://raw.githubusercontent.com/xyzstore/new7/main/"
start=$(date +%s)
secs_to_human() {
...
clear
print_install "Membuat Password Default SSH"
default_pass="ARI-$(tr -dc A-Za-z0-9 </dev/urandom | head -c 8)"
echo "$default_pass" >/root/password-default.txt
print_success "Password Default SSH"
}
clear
function pasang_ssl() {
clear
...
# Hardening: pastikan UseDNS dan GSSAPIAuthentication off
# supaya login SSH tidak timeout akibat reverse DNS lookup
...
# SETUP DNS
# ==========================================
# Pastikan resolv.conf tidak immutable sebelum diubah
...
# Jangan lock resolv.conf dan jangan hidupkan kembali systemd-resolved
# yang sudah di-disable — kombinasi itu menyebabkan DNS broken setelah reboot
}
instal
echo ""
