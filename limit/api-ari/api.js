...
function parseSSHOutput(output) {
    ...
    return {
        username: extract(/(?:Remark|Remarks|Username)\s+:\s+(\S+)/),
...
function isValidUsername(user) {
    return typeof user === "string" && /^[A-Za-z0-9_-]+$/.test(user);
}

function usernameExistsEverywhere(user) {
    const dbFiles = [
        "/etc/ssh/.ssh.db",
        "/etc/vmess/.vmess.db",
        "/etc/vless/.vless.db",
    ...
    try {
        ...
        if (passwd.split("\n").some(line => line.split(":")[0] === user)) return true;
    } catch (_) {}

    for (const file of dbFiles) {
        try {
            const content = fs.readFileSync(file, "utf8");
            if (content.split("\n").some(line => line.trim().split(/\s+/)[1] === user)) return true;
        } catch (_) {}
    }

    const limitFiles = [
        `/etc/kyt/limit/ssh/ip/${user}`,
        `/etc/kyt/limit/vmess/ip/${user}`,
        `/etc/kyt/limit/vless/ip/${user}`,
        `/etc/kyt/limit/trojan/ip/${user}`,
        `/etc/kyt/limit/shadowsocks/ip/${user}`,
    ];

    return limitFiles.some(file => fs.existsSync(file));
}

function validateCreateUsername(res, user) {
    if (!isValidUsername(user)) {
        res.status(400).json({ status: "error", message: "Username hanya boleh berisi huruf, angka, underscore (_), dan strip (-)." });
        return false;
    }
    if (usernameExistsEverywhere(user)) {
        res.status(409).json({ status: "error", message: `Username ${user} sudah ada di salah satu protokol. Pembuatan akun ditolak.` });
        return false;
    }
    return true;
}
...
function runScript(script, args, callback) {
    execFile("bash", [script, ...args], { cwd: __dirname }, callback);
}
...
app.get("/createssh", (req, res) => {
    ...
    if (!validateCreateUsername(res, user)) return;

    // Eksekusi skrip shell untuk membuat akun SSH
    runScript("create_ssh.sh", [user, password, iplimit, exp], (error, stdout, stderr) => {
        if (error) {
...
app.get("/createvmess", (req, res) => {
    ...
    // Eksekusi skrip shell untuk membuat akun SSH
    runScript("create_vmess.sh", [user, exp, iplimit, quota], (error, stdout, stderr) => {
        if (error) {
            return res.json({ status: "error", message: stderr });
        }
...
app.get("/createvless", (req, res) => {
    ...
    // Eksekusi skrip shell untuk membuat akun SSH
    runScript("create_vless.sh", [user, exp, iplimit, quota], (error, stdout, stderr) => {
        if (error) {
            return res.json({ status: "error", message: stderr });
        }
...
app.get("/createtrojan", (req, res) => {
    ...
    // Eksekusi skrip shell untuk membuat akun SSH
    runScript("create_trojan.sh", [user, exp, iplimit, quota], (error, stdout, stderr) => {
        if (error) {
            return res.json({ status: "error", message: stderr });
        }
...
app.get("/createshadowsocks", (req, res) => {
    ...
    // Eksekusi skrip shell untuk membuat akun SSH
    runScript("create_ss.sh", [user, exp, iplimit, quota], (error, stdout, stderr) => {
        if (error) {
            return res.json({ status: "error", message: stderr });
        }
